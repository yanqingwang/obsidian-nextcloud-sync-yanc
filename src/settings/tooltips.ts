...
export const SIGN_IN_HELP =
  'Two ways to sign in: (A) Log in via browser (recommended) fills Username and App password for you; or (B) enter Username + paste an App password manually — they are alternatives, not both. After manual sign-in, press "Test connection" once: it checks reachability and credentials in two stages and activates syncing when both pass (on hostile mobile networks, e.g. HarmonyOS 出境易, it also pinpoints exactly which link of the chain failed).';
